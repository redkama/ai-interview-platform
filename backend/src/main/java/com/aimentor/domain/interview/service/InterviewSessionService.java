...
public class InterviewSessionService {
    ...
    public InterviewSessionResponse startSession(Long userId, StartInterviewSessionRequest request) {
        ...
        List<AiQuestionItem> generatedQuestions = safelyGenerateInterviewQuestions(savedSession, request.questionCount());
    ...
    private void refreshSessionFeedback(InterviewSession interviewSession) {
        ...
        if (answeredQuestions.isEmpty()) {
            replaceSessionFeedback(interviewSession, 0, 0, 0, 0, "아직 제출된 답변이 없습니다.");
            return;
        }
        ...
        String reportSummary = safelyGenerateReportSummary(interviewSession, feedbackItems);
        replaceSessionFeedback(interviewSession, relevanceScore, logicScore, specificityScore, overallScore, reportSummary);
    }
    ...
    private void replaceSessionFeedback(
        ...
        if (currentFeedback != null) {
            currentFeedback.update(relevanceScore, logicScore, specificityScore, overallScore, summary);
            return;
        }
    ...
    private String safelyGenerateReportSummary(
            InterviewSession interviewSession,
            List<AiAnalyzeAnswerFeedbackResponse> feedbackItems
    ) {
        try {
            String summary = aiIntegrationService.generateReportSummary(
        ...
        return "'" + interviewSession.getTitle() + "' 세션은 AI 리포트 생성을 사용할 수 없어 기본 요약으로 대체되었습니다.";
    }
